...
__global__ void farthest_point_sampling_kernel(int b, int n, int m,
    const int16_t *__restrict__ dataset, float *__restrict__ temp, int *__restrict__ idxs) {
    // dataset: (B, N, 3)
    // tmp: (B, N)
    // output:
    //      idx: (B, M)
    ...
    int16_t batch_index = blockIdx.x;
    ...
    int besti = 0;
    float best = -1;
    int16_t x1 = dataset[old * 3 + 0];
    int16_t y1 = dataset[old * 3 + 1];
    int16_t z1 = dataset[old * 3 + 2];
...
void farthest_point_sampling_kernel_launcher(int b, int n, int m,
    const int16_t *dataset, float *temp, int *idxs) {
    // dataset: (B, N, 3)
    // tmp: (B, N)
    // output:
    //      idx: (B, M)
